...
# _________________________________________________________________________________________________________________________________________________________________ #
    ...
    async def ping(self, interaction: Interaction):
        await interaction.send(f"`Discord Latency    :` {round(self.client.latency * 1000)}ms")
...
# _________________________________________________________________________________________________________________________________________________________________ #
    ...
    async def user_banner(self, interaction: Interaction, member: nextcord.Member):
        ...
        try: banner = member.banner.url
        except: return await interaction.send(f"`{member.display_name}` has no banner or has a guild banner.")
    ...
    async def user_info(self, interaction: Interaction, member: nextcord.Member):
        ...
        embed = nextcord.Embed(title=f"{member.name}#{member.discriminator}", description=''.join(infoParse), colour=int(os.getenv("COL"), 16))
