...
class SlashAni(commands.Cog):
    ...
    async def anilist_aniset(self, interaction: Interaction, anilist_username: str = SlashOption("anilist_username",
                                                                                                 description="Your anilist username",
                                                                                                 required=True)):
        user = await sql_func.sql.fetchval("SELECT anilist_name FROM anilist WHERE discord_id = $1", interaction.user.id)
        ...
        if user is None:
            await sql_func.sql.execute(
                "INSERT INTO anilist(discord_id, discord_name, anilist_name, anilist_id, anilist_url) VALUES ($1, $2, $3, $4, $5)",
                interaction.user.id, interaction.user.name, anilist['name'], anilist['id'], anilist['url'])
        else:
            await sql_func.sql.execute(
                "UPDATE anilist SET discord_name = $1, anilist_name = $2, anilist_id = $3, anilist_url = $4 WHERE discord_id = $5",
                interaction.user.name, anilist['name'], anilist['id'], anilist['url'], interaction.user.id)
        await interaction.send(embed=embed, ephemeral=True)

    # [+] View User Information [+]#
    @anilist.subcommand(name="user", description="View information about an User")
    async def anilist_user(self, interaction: Interaction, username: str = SlashOption("username",
                                                                                       description="Name Of User || KEEP BLANK IF YOU WANT TO VIEW YOURSELF & IN THE DB",
                                                                                       required=False)):
        if username is None:
            user = await sql_func.sql.fetchval("SELECT anilist_name FROM anilist WHERE discord_id = $1",
                                               interaction.user.id)
            if user is None:
                return await interaction.send(
                    "Looks like you're not in the database. Please use /aniset to set your account!", ephemeral=True)
        else:
            user = username
    ...
    async def anilist_anime(self, interaction: Interaction,
                            anime_name: str = SlashOption("anime_name", description="Name of the Anime",
                                                          required=True)):
        members = [];
        dbMembs = []

        try:
            db = await sql_func.sql.fetch("SELECT anilist_name, discord_id FROM anilist")
        except:
            db = []
    ...
    async def anilist_manga(self, interaction: Interaction,
                            manga_name: str = SlashOption("manga_name", description="Name of the Manga",
                                                          required=True)):
        members = [];
        dbMembs = []
    ...
    async def anilist_affinity(self, interaction: Interaction):
        ...
        who_used_comm = interaction.user.id
        db = await sql_func.sql.fetch("SELECT anilist_name, discord_id FROM anilist")
        members = []
        dbMembs = []
        affinityList = []
        ...
        # Add a fake user to the end of the list, so once all users have been looped through it wipes the array for
        # more usage
        ...
        if who_used_comm == interaction.user.id:
            return await interaction.send("You have not set your anilist with the bot. Please use /anilist aniset",
                                          ephemeral=True)
        ...
        for member in dbMembs:
            ...
            affinityList.append(
                f"**{affinity['affinity']}%** with **[{affinity['name']}]({affinity['url']})** - [*{affinity['shares']} media shares*]")

        # Well, this works and doesn't (negatives show up at the top)
        affinityList.sort(key=natural_keys)
        affinityList.reverse()
    ...
    async def anilist_staff(self, interaction: Interaction,
                            staff_name: str = SlashOption("staff_name", description="Name of staff member",
                                                          required=True)):
        pass
