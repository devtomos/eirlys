        ...
        if members != []:
            for member in members:
                ...
                if uStats['status'] == 'REPEATING':
                    info['repeating'].append(f"{member} - `{uStats['progress']} | {uStats['score']}/10`")

                if uStats['status'] == 'COMPLETED':
                    if uStats['repeat'] != 0:
                        info['completed'].append(f"{member} - `{uStats['repeat']} repeats | {uStats['score']}/10`")
                    else:
                        info['completed'].append(f"{member} - `{uStats['score']}/10`")

                if uStats['status'] == 'CURRENT':
                    info['current'].append(f"{member} - `{uStats['progress']} current | {uStats['score']}/10`")

                if uStats['status'] == 'DROPPED':
                    info['dropped'].append(f"{member}")

                if uStats['status'] == 'PAUSED':
                    info['paused'].append(f"{member}")
